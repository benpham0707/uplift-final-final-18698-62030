...
/**
 * @deprecated Use WorkshopItemWithTeaching instead
 * Legacy interface from before Phase 17-19-20 pipeline separation
 */
export interface CoachingIssue {
  id: string;
  category: RubricCategory;
  title: string;
  problem: string; // LEGACY - use teaching.problem instead
...
export interface AnalysisResult {
  ...
  workshopItems?: Array<{
    ...
    quote: string;
    problem: string; // LEGACY - replaced by teaching.problem
    why_it_matters: string; // LEGACY - replaced by teaching.problem.whyItMatters
    suggestions: Array<{
      text: string;
      rationale: string; // LEGACY - replaced by teaching.suggestionRationales
...
/**
 * Workshop Item from Phase 17 (Updated - No longer includes problem/why_matters/severity)
 *
 * Phase 17 now generates ONLY: id, quote, rubric_category, suggestions
 * Phase 19 adds: teaching (problem/impact explanations)
 * Phase 20 adds: suggestionRationales (per-suggestion "why this works")
 */
export interface WorkshopItemWithTeaching {
  ...
  // Phase 17 suggestions (3 types: polished_original, voice_amplifier, divergent_strategy)
  suggestions: Array<{
    text: string;
    type: 'polished_original' | 'voice_amplifier' | 'divergent_strategy';
  }>;

  // LEGACY FIELDS (may exist in old analyses, but no longer generated by Phase 17)
  problem?: string; // Deprecated - use teaching.problem instead
  why_it_matters?: string; // Deprecated - use teaching.problem.whyItMatters instead
