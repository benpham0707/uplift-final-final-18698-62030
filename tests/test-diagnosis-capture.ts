...
async function runTest() {
  console.log('🧪 TESTING: Phase 11 (Captured Diagnosis)');
  ...
  try {
    ...
    console.log('\n' + '='.repeat(80));
    console.log('🔍 CHECKING DIAGNOSES');
    console.log('='.repeat(80));
    
    result.workshopItems.forEach((item, i) => {
      console.log(`\n### Issue ${i + 1}: ${item.rubric_category}`);
      console.log(`Quote: "${item.quote}"`);
      // This should now be the specific diagnosis from the LLM
      console.log(`Why It Matters (SPECIFIC): ${item.why_it_matters}`); 
    });
